...
from quad_control.Controller import Controller
from quad_input_interfacing.InputInterface import InputInterface
from quad_control.State import State, BehaviorState
from quad_control.Kinematics import four_legs_inverse_kinematics
from quad_control.Config import Configuration
from quad_control.msg import TaskSpace, JointSpace, Angle
from std_msgs.msg import Bool

if is_physical:
    from quad_servo_interfacing.HardwareInterface import HardwareInterface
    from quad_peripheral_interfacing.IMU import IMU
    from quad_control.Config import Leg_linkage
...
class DingoDriver:
    def __init__(self,is_sim, is_physical, use_imu):
        ...
        if self.is_sim:
            self.sim_command_topics = ["/quad_controller/FR_theta1/command",
                    "/quad_controller/FR_theta2/command",
                    "/quad_controller/FR_theta3/command",
                    "/quad_controller/FL_theta1/command",
                    "/quad_controller/FL_theta2/command",
                    "/quad_controller/FL_theta3/command",
                    "/quad_controller/RR_theta1/command",
                    "/quad_controller/RR_theta2/command",
                    "/quad_controller/RR_theta3/command",
                    "/quad_controller/RL_theta1/command",
                    "/quad_controller/RL_theta2/command",
                    "/quad_controller/RL_theta3/command"]
...
def main():
    """Main program
    """
    rospy.init_node("quad_driver") 
    signal.signal(signal.SIGINT, signal_handler)
    quad = DingoDriver(is_sim, is_physical, use_imu)
    quad.run()
